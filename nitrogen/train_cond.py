...
class BattleBellmanCacheDataset(IterableDataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.game = str(game)
        self.require_conditioning = bool(require_conditioning)
    ...
    def __iter__(self) -> Iterator[Dict[str, Any]]:
        ...
        for fp in files:
            ...
            values: Optional[torch.Tensor] = payload.get("values", None)

            # conditioning (from your existing precache payload)
            states: Optional[torch.Tensor] = payload.get("states", None)  # [N, 3]
            chips: Optional[torch.Tensor] = payload.get("chips", None)  # [N]
            ...
            if self.require_conditioning:
                if states is None or chips is None:
                    raise RuntimeError(
                        f"Cache file missing conditioning in {fp.name}. "
                        f"Expected keys: 'states' ([N,3]) and 'chips' ([N])."
                    )
                if int(states.shape[0]) != n or int(chips.shape[0]) != n:
                    raise RuntimeError(
                        f"Conditioning length mismatch in {fp.name}: "
                        f"frames N={n}, states={None if states is None else tuple(states.shape)}, "
                        f"chips={None if chips is None else tuple(chips.shape)}"
                    )
                if int(states.shape[1]) != 3:
                    raise RuntimeError(f"Expected states shape [N,3] in {fp.name}, got {tuple(states.shape)}")
            ...
            for i in order:
                ...
                # vision_horizon=1 => repeat is harmless and keeps API stable
    ...
    def __call__(self, samples: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for s in samples:
            ...
            d: Dict[str, Any] = {
                ...
                "dropped_frames": s["dropped_frames"].unsqueeze(0)
                if s["dropped_frames"].ndim == 1
                else s["dropped_frames"],
                "game": s["game"],
            }
            ...
            if enc:
                encoded.append(enc)
                values.append(
                    s["value"] if torch.is_tensor(s["value"]) else torch.as_tensor(s["value"], dtype=torch.float32)
                )
                games.append(s["game"])
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="0 => auto from manifest (abs_p90 median) if available.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not down-weight negative values (clamp value>=0).",
    )
    ...
    parser.add_argument(
        "--no_conditioning",
        action="store_true",
        help="Disable state/chip conditioning (will ignore cache 'states'/'chips' if present).",
    )
